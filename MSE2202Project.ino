...
void loop() {
  ...
  switch (ModeIndex) {
    ...
    case 1: /**********************************mode 1 base = look */
      ...
      else if (((analogRead(HallRgt) - NOFIELDRGT) > 5 ) || ((analogRead(HallRgt) - NOFIELDRGT) < -5 )) {
        PickUp(0);
      }
...
*/
  ...
  while (true) {
    ...
    if ((analogRead(GripLight) <= 920) || (analogRead(GripLight) >= 980)) { // Light
